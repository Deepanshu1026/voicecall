...
<head>
    <meta charset="UTF-8">
    <title>Agent Requests - Avisa Portal</title>
    ...
    <style>
        ...
        .timeline-item::before {
            ...
            height: 12px;
            border-radius: 50%;
            background: var(--timeline-color, #0d6efd);
        ...
        #statsTrigger {
            ...
            /* transform is handled by JS for dragging, so we remove translateY(-50%) from CSS to avoid conflict or manage it carefully */
            transform: translateY(-50%); 
...
function updateStats(apps) {
    ...
    // Use local date for 'Today' check to match user's timezone
...
function renderDetails(app, logs) {
    ...
    html += `<tr><td class="text-muted">Submitted By:</td><td><i class="bi bi-person-circle text-secondary me-1"></i> ${app.agent_name || 'Unknown'}</td></tr>`;
...
function renderTimeline(logs) {
    ...
    container.innerHTML = logs.map(log => {
        const isCaseActivity = log.log_source === 'case_activity';
        const badge = isCaseActivity ? '<span class="badge bg-light text-dark border ms-2" style="font-size:0.7em">CASE</span>' : '';
        const iconColor = isCaseActivity ? '#6610f2' : '#0d6efd'; // Purple for case, Blue for app
        
        return `
        <div class="timeline-item" style="--timeline-color: ${iconColor}">
            <div class="fw-bold text-capitalize d-flex align-items-center">
                ${log.action_type.replace(/_/g, ' ')}
                ${badge}
            </div>
            <div class="small text-muted">by ${log.user_name || 'System'} (${log.user_role || 'System'})</div>
