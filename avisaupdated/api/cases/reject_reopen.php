...
try {
    ...
    if (!in_array($_SESSION['role'], ['manager', 'admin'])) {
        ob_end_clean();
        die(json_encode(["success" => false, "error" => "Only managers or admins can reject reopen requests"]));
    }
    ...
    if ($case['status'] !== 'reopen-requested') {
        ob_end_clean();
        die(json_encode(["success" => false, "error" => "Case is not awaiting reopen approval"]));
    }
    
    // Change status back to completed
    $updateStmt = $db->prepare("UPDATE cases SET status = 'completed' WHERE id = ?");
    $updateStmt->bind_param("i", $caseId);
    $updateStmt->execute();
    
    // Log activity
    log_case_activity($caseId, $userId, "reopen_rejected", [
        "reason" => $reason,
        "rejected_by" => $userId
    ]);
    ...
    die(json_encode([
        "success" => true,
        "message" => "Reopen request rejected. Case remains completed.",
        "case_id" => $caseId,
        "status" => "completed"
    ]));
