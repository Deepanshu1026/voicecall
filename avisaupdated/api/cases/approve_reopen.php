...
try {
    ...
    if (!in_array($_SESSION['role'], ['manager', 'admin'])) {
        ob_end_clean();
        die(json_encode(["success" => false, "error" => "Only managers or admins can approve reopen"]));
    }
    ...
    if ($case['status'] !== 'reopen-requested') {
        ob_end_clean();
        die(json_encode(["success" => false, "error" => "Case is not awaiting reopen approval. Current status: " . $case['status']]));
    }
