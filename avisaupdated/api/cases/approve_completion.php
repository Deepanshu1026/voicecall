...
try {
    ...
    if (!in_array($_SESSION['role'], ['manager', 'admin'])) {
        ob_end_clean();
        die(json_encode(["success" => false, "error" => "Only managers or admins can approve completion"]));
    }
    ...
    if ($case['status'] !== 'awaiting-completion-approval') {
        ob_end_clean();
        die(json_encode(["success" => false, "error" => "Case is not waiting for completion approval"]));
    }
    ...
    // Log activity
    log_case_activity($caseId, $userId, "case_completed", [
        "approved_by" => $userId
    ]);
