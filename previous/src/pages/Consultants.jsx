...
const ConsultantCard = memo(({ consultant, isOnline, onStartChat, onStartCall }) => {
  ...
  const language = consultant.language || 'English, Hindi';
  const experience = consultant.experience || '5';
  const clients = consultant.clients || '100';

  return (
    <div className="card">
      <div className={`ribbon ${isOnline ? 'golden' : ''}`}>
        {isOnline ? 'Golden' : 'Silver'}
      </div>
      <div className="firstcardp">
        <img
          src={avatar}
          alt={name}
        ...
        />
        <p className="flex-stars">
          <span className="stars">★★★★★</span>
        </p>
        <p className="second-cls-pe">{clients} Clients</p>
        {!isOnline && (
      ...
      <div>
        <div className="flex-username">
          <h5>{name}</h5>
          <div className="verification-ticks">
            {isOnline ? (
              <svg className="verify-tickk" xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 30 30" fill="none">
                <g>
        ...
        <div className="btn-container">
          <button
            className={`btn btn-outline-success ${isOnline ? 'online-btn' : 'offline-btn'}`}
            disabled={!isOnline}
...
const Consultants = () => {
  ...
  const jsonLd = {
    ...
    '@type': 'WebPage',
    name: 'Our Visa Consultants',
    description:
      'Connect with experienced visa and immigration consultants at A Visa Experts. Get personalized guidance via chat, video, or call.',
    url: 'https://avisaexperts.com/consultants',
  };

  return (
    <LandingLayout>
      <SEO
        title="Our Visa Consultants | A Visa Experts Advisors"
        description="Connect with experienced visa and immigration consultants. Book a free consultation and get personalized guidance for your visa application."
        keywords="visa consultants, immigration advisors, Kaveesh Kapoor, visa expert consultation, immigration lawyer"
        canonicalPath="/consultants"
        ogImage="/images/user/kaveesh.webp"
        jsonLd={jsonLd}
      />
