...
class _GlobalAppBarState extends State<GlobalAppBar> {
  ...
  static String? _globalPollingUserId;
  static bool _isGlobalPollingActive = false;
  String? _instanceUserId;
  ...
  Future<void> _initializePolling() async {
    ...
    if (userIdValue is String) {
      _instanceUserId = userIdValue;
    } else if (userIdValue is int) {
      _instanceUserId = userIdValue.toString();
    }
    ...
    // Fetch ticket count immediately.
    ...
    // Fetch wallet balance immediately so the chip shows the real value without
    // waiting for the user to open the wallet page.
    final balance = await fetchWalletBalanceFromApi(_instanceUserId!);
    if (mounted && walletBalanceNotifier.value != balance) {
      walletBalanceNotifier.value = balance;
    }
    WalletService.restartPolling(userId: _instanceUserId!);
  ...
  Widget build(BuildContext context) {
    ...
    return AppBar(
      ...
      actions: [
        _WalletChip(
          balanceNotifier: walletBalanceNotifier,
...
class _WalletChip extends StatelessWidget {
  final ValueNotifier<double> balanceNotifier;
  final VoidCallback onTap;

  const _WalletChip({
    required this.balanceNotifier,
    required this.onTap,
  });
  ...
  Widget build(BuildContext context) {
    return ValueListenableBuilder<double>(
      ...
      builder: (context, balance, _) => Padding(
        padding: const EdgeInsets.only(right: 4.0),
        child: Tooltip(
          message: 'Wallet: ₹${balance.toStringAsFixed(0)}',
          child: Material(
                  ...
                  children: [
                    ...
                    AnimatedSwitcher(
                      ...
                      child: Text(
                        ...
                        key: ValueKey<String>(_formatBalance(balance)),
                        style: const TextStyle(
                          color: Colors.white,
