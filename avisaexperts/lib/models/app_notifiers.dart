...
// Global notifier to hold the current wallet balance. Initialize to 0.
final ValueNotifier<double> walletBalanceNotifier = ValueNotifier(0.0);
...
Future<int> fetchTicketCountFromApi(String userId) async {
...
Future<int> fetchUnreadMessageCountFromApi(String userId) async {
  ...
  final url = Uri.parse('${AppConfig.inbox}?receiver_id=$userId');
  log(
...
// ✅ NEW: Helper function to fetch wallet balance from API
Future<double> fetchWalletBalanceFromApi(String userId) async {
  final url = Uri.parse('${AppConfig.wallet}?user_id=$userId');
  log("fetchWalletBalanceFromApi: Fetching for User ID: $userId from $url");
  ...
  try {
    ...
    if (response.statusCode == 200) {
      if (response.body.trim().isEmpty) {
        log("fetchWalletBalanceFromApi: Empty response body.");
        return 0.0;
      }
      final decodedResponse = jsonDecode(response.body);
      if (decodedResponse is Map<String, dynamic>) {
        final balance = (decodedResponse['balance'] ?? 0).toDouble();
        log("fetchWalletBalanceFromApi: Balance fetched: $balance");
        return balance;
      }
      return 0.0;
    } else {
      log("fetchWalletBalanceFromApi: Server error (Code: ${response.statusCode})");
      return 0.0;
    }
  } catch (e) {
    log("fetchWalletBalanceFromApi: Error fetching balance: $e");
    return 0.0;
  }
}

// ✅ NEW: Global service to poll and update wallet balance
class WalletService {
  static Timer? _pollingTimer;
  static bool _isPolling = false;
  static String? _currentUserId;

  static Future<void> startPolling({Duration interval = const Duration(seconds: 30)}) async {
    final prefs = await SharedPreferences.getInstance();
    final dynamic userIdValue = prefs.get(USER_ID_PREFS_KEY) ??
        prefs.get('user_id') ??
        prefs.get('userId');
    String? userId;
    if (userIdValue is int) {
    ...
    if (userId == null || userId.trim().isEmpty || userId == '0') {
      log("WalletService: No/invalid user ID found in prefs, will not start polling");
      return;
    }
    await restartPolling(userId: userId, interval: interval);
  }

  static Future<void> restartPolling({required String userId, Duration interval = const Duration(seconds: 30)}) async {
    if (_isPolling && _currentUserId == userId) {
      log("💰 WalletService: Polling already active for user $userId");
      return;
    }
    ...
    log("💰 WalletService: Starting polling every ${interval.inSeconds} seconds for user $_currentUserId");
    await _fetchAndUpdateBalance();

    _pollingTimer = Timer.periodic(interval, (timer) {
      _fetchAndUpdateBalance();
    });
  }
  ...
  static void stopPolling() {
    ...
    _isPolling = false;
    _currentUserId = null;
    log("⏹️ WalletService: Stopped polling");
  }

  static Future<void> _fetchAndUpdateBalance() async {
    if (_currentUserId == null || _currentUserId!.trim().isEmpty) return;
    try {
      final balance = await fetchWalletBalanceFromApi(_currentUserId!);
      if (walletBalanceNotifier.value != balance) {
        walletBalanceNotifier.value = balance;
        log("💰 WalletService: Updated balance to $balance");
      }
    } catch (e) {
      log("❌ WalletService: Error updating balance: $e");
    }
  }
  ...
  static void bindSocketEvents() {
    ChatSocketService().onEvent('wallet:updated', (data) {
      if (data is Map<String, dynamic>) {
        final balance = (data['balance'] ?? 0).toDouble();
        updateBalance(balance);
        log("💰 WalletService: wallet:updated balance to $balance");
      }
    });
    ...
    ChatSocketService().onEvent('call:charged', (data) {
      if (data is Map<String, dynamic> && data['balance'] != null) {
        ...
        updateBalance(balance);
        log("💰 WalletService: call:charged balance to $balance");
      } else {
        refreshBalance();
      }
    });
