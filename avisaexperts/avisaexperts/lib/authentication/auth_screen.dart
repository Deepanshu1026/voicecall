...
class _AuthScreenState extends State<AuthScreen> {
  ...
  Future<void> _performLogin() async {
    ...
    try {
      ...
      request.fields['login_input'] = identifier;
  ...
  Future<void> _performConsultantLogin() async {
    ...
    try {
      final url = Uri.parse(
          'https://avisaexperts.com/appagentlogin.php?useremail=${Uri.encodeComponent(email)}&password=${Uri.encodeComponent(password)}');
  ...
  Future<void> handleLoginSuccessFCM(int userId) async {
          ...
          if (responseData['status'] == 'success') {
            if (isLogin) {
              ...
              if (userData != null && userData['user_id'] != null) {
                ...
                debugPrint(
                    "Login Successful! User ID: ${userData['user_id']} saved. Profile Path: ${userData['user_profile']}");

                // --- CALL FCM TOKEN SAVE FUNCTION HERE ---
                if (mounted) await handleLoginSuccessFCM(userData['user_id']);
                // --- END FCM TOKEN LOGIC ---
                  ...
                  onPressed: isUpdating
                      ...
                      : () async {
                          ...
                          try {
                            ...
                            final url = Uri.parse(
                                'https://avisaexperts.com/appupdatecountrycode.php?user_id=$userId&country_code=$countryCode');
