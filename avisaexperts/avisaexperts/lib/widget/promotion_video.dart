...
class _FloatingPromotionVideoState extends State<FloatingPromotionVideo> {
  ...
  Future<void> _fetchInstagramVideoUrl() async {
    ...
    try {
      // Fetch more items so we can skip regular posts and pick only reels.
      final response = await http.get(
        Uri.parse(
          'https://graph.instagram.com/me/media?fields=media_url,media_type,permalink&access_token=$_instagramToken&limit=10',
        ),
      );
      if (response.statusCode == 200) {
        final data = json.decode(response.body);
        if (data['data'] != null && data['data'].isNotEmpty) {
          // Look for the first item that is a VIDEO AND has a /reel/ permalink.
          final mediaList = data['data'] as List<dynamic>;
          for (final media in mediaList) {
            if (media['media_type'] == 'VIDEO') {
              final permalink = media['permalink']?.toString() ?? '';
              if (permalink.contains('/reel/')) {
